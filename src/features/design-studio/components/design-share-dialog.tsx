...
import type { DesignShareData } from "@/features/design-studio/types";
...
export function DesignShareDialog({
  ...
  async function createShare() {
    setCreating(true);
    try {
      const res = await fetch(`/api/design-previews/${previewId}/shares`, {
        method: "POST",
  ...
  async function revokeShare(shareId: string) {
    const res = await fetch(`/api/design-previews/${previewId}/shares/${shareId}`, { method: "DELETE" });
    if (!res.ok) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Share Design Preview</DialogTitle>
          <DialogDescription>
            Generate a secure, expiring link the customer can use to review and approve this design, no login
            required.
          </DialogDescription>
        </DialogHeader>
