...
import { noteCategoryOptions } from "@/lib/validations/appointment";
...
export function AppointmentNotesPanel({ appointmentId, notes }: { appointmentId: string; notes: AppointmentNoteItem[] }) {
            ...
            <li key={note.id} className="rounded-xl border border-border p-3">
              <div className="mb-1 flex items-center justify-between">
                <Badge variant="outline" className="font-normal text-muted-foreground">
                  {noteCategoryOptions.find((o) => o.value === note.category)?.label ?? note.category}
                </Badge>
